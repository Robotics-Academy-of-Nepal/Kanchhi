...
context_keywords = {
    ...
    "founder" : ["founder","founded","founder president"],
    "achievements" : ["achievement","achievements"],
    "introduction" : ["introduce","introduction","explain","define","information","more information","what is CAN"]
}
...
while True:
    ...
    def contains_any_words(text, word_list):
        text = text.lower().translate(str.maketrans('', '', string.punctuation))
        text_words = set(text.split())
        word_set = set(word_list)
        return bool(text_words & word_set)
    

    def recognize_wakeup_words():
    ...
    wakeup_text = recognize_wakeup_words()
    ...
    wakeup_condition = contains_any_words(translated_text,wake_up_words)
    print(wakeup_condition)
    ...
    if wakeup_condition:
        ...
        while True:
            ...
            history.append({"role": "user", "content": translated_text})
            stream = ollama.chat(
                    model='llama3',
                    messages=history,
                    stream=True,
            ...
            print(f"कान्छी robot: {full_response}")
            ...
            try:
                ...
                print("कान्छी robot:", final_response)
            except Exception as e:
                print("An error occured:", e)
            ...
            def recognize_from_microphone():
                ...
                if speech_recognition_result.reason == speechsdk.ResultReason.RecognizedSpeech:
                    ...
                    print("Error while trasncribing the audio!!")
            ...
            def contains_any_exit_words(text, word_list):
            ...
            def handle_user_input(user):
                global context
                if contains_any_words(user, can_words):
                    context["is_about_can"] = True
                    context["last_topic"] = "CAN"
                    return detect_context(user,context_keywords,answers)
                elif context["is_about_can"]:
                    if context["last_topic"] == "CAN":
                        return detect_context(user,context_keywords,answers)

            text = handle_user_input(transcribed_text)
            exit_condition = contains_any_exit_words(translated_text,sleeping_words)
            
            if exit_condition:
                wav_list = [
                    "exit_audios\message_1.wav",
                    "exit_audios\message_2.wav",
                    "exit_audios\message_3.wav",
                    "exit_audios\message_4.wav"
                ]
